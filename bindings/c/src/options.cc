...
{
  ARG_NOT_NULL(options_handle, err_info_container);
  ARG_NOT_NULL(argv, err_info_container);
...
VW_DLL_PUBLIC VWStatus vw_create_options_from_command_line_cstring(
    const char* commandLine, VWOptions** options_handle, VWErrorInfo* err_info_container) noexcept
try
{
  ARG_NOT_NULL(options_handle, err_info_container);
  ARG_NOT_NULL(commandLine, err_info_container);
  ...
  return vw_create_options_from_command_line(argc, argv, options_handle, err_info_container);
}
CATCH_RETURN(err_info_container)
...
try
{
  ARG_NOT_NULL(option_name, err_info_container);
  ARG_NOT_NULL(option_value, err_info_container);
