...
VW_DLL_PUBLIC vw_status example_push_feature_space_index(vw_example* c_example, int value)
{
...
VW_DLL_PUBLIC vw_status example_get_feature_space_copy(
...
VW_DLL_PUBLIC vw_status example_assign_feature_space(
...
VW_DLL_PUBLIC vw_status feature_space_copy(const vw_feature_space* c_features_source, vw_feature_space* c_features_dest)
{
...
VW_DLL_PUBLIC vw_status allocate_feature_space(vw_feature_space** c_features)
{
  *c_features = reinterpret_cast<vw_feature_space*>(new features);
  return vw_success;
}

VW_DLL_PUBLIC vw_status deallocate_feature_space(vw_feature_space* c_features)
{
...
VW_DLL_PUBLIC vw_status feature_space_push_feature(vw_feature_space* c_features, uint64_t ft_index, float ft_value)
{
  auto* fs = reinterpret_cast<features*>(c_features);
...
VW_DLL_PUBLIC vw_status example_get_feature_offset(const vw_example* c_example, int* feature_offset)
{
  const auto* ex = reinterpret_cast<const example*>(c_example);
...
VW_DLL_PUBLIC vw_status example_set_feature_offset(vw_example* c_example, int feature_offset)
{
  auto* ex = reinterpret_cast<example*>(c_example);
...
VW_DLL_PUBLIC vw_status example_get_tag(const vw_example* c_example, const char** tag)
{
  const auto* ex = reinterpret_cast<const example*>(c_example);
  *tag = &ex->tag[0];
  return vw_success;
...
VW_DLL_PUBLIC vw_status example_set_tag(vw_example* c_example, const char* tag, int length)
{
  auto* ex = reinterpret_cast<example*>(c_example);
  ex->tag.clear();
...
VW_DLL_PUBLIC vw_status example_get_weight(const vw_example* c_example, float* weight)
{
  const auto* ex = reinterpret_cast<const example*>(c_example);
  *weight = ex->weight;
  return vw_success;
...
VW_DLL_PUBLIC vw_status example_set_weight(vw_example* c_example, float weight)
{
  auto* ex = reinterpret_cast<example*>(c_example);
  ex->weight = weight;
  return vw_success;
