...
extern "C"
{
  ...
  typedef void (*trace_message_func)(void*, int trace_level, const char*, size_t);
  ...
  vw_status delete_options(vw_options*);

  vw_status initialize(
      vw_options*, bool skipModelLoad, trace_message_func trace_listener, void* trace_context, vw_workspace** output);
  vw_status initialize_with_model(vw_options*, void* context, read_func writer, bool skipModelLoad,
      trace_message_func trace_listener, void* trace_context, vw_workspace** output);
  vw_status seed_vw_model(
      const vw_workspace*, vw_options* opts, trace_message_func trace_listener, void* trace_context, vw_workspace**);
  ...
  vw_status finish(vw_workspace* all);
  vw_status delete_vw(vw_workspace* all);

  vw_status vw_get_pred_type(vw_workspace* vw, vw_prediction_type*);
  vw_status vw_get_label_type(vw_workspace* vw, vw_label_type*);

  vw_status learn(vw_workspace* vw, vw_example* example);
  vw_status learn_multiline(vw_workspace* vw, vw_example* example, size_t len);

  vw_status predict(vw_workspace* vw, vw_example* example);
  vw_status predict_multiline(vw_workspace* vw, vw_example* example, size_t len);

  // finish one or more examples? How do we handle multi_ex?
  vw_status finish_example(vw_workspace*, vw_example*);
  vw_status finish_example_multiline(vw_workspace*, vw_example*, size_t);

  // End pass -> all.passes_complete++, in_pass_counter = 0; (something?)
  vw_status end_pass(vw_workspace* vw);
