...
extern "C"
{
  ...
  VW_DLL_PUBLIC VWErrorInfo* vw_create_error_info() VW_API_NOEXCEPT;
  VW_DLL_PUBLIC void vw_destroy_error_info(VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC const char* vw_error_info_get_message(const VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  ...
  VW_DLL_PUBLIC VWStatus vw_workspace_save_model(VWWorkspace* workspace_handle, void* context,
      VWWriteFunc* writer, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  ...
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_audit(
      const VWWorkspace* workspace_handle, bool* audit, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_input_type(
      const VWWorkspace* workspace_handle, VWInputType* input_type, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_hash_type(
      const VWWorkspace* workspace_handle, VWHashType* hash_type, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_hash_seed(
      const VWWorkspace* workspace_handle, uint64_t* hash_seed, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_num_bits(
      const VWWorkspace* workspace_handle, uint64_t* num_bits, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_configured_hasher(
      const VWWorkspace* workspace_handle, VWHasher** hasher, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  ...
  VW_DLL_PUBLIC VWStatus vw_hash_all(
      const uint8_t* data, size_t length, uint64_t seed, uint64_t*, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_hash_string(
      const char* data, size_t length, uint64_t seed, uint64_t*, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;

  // Can also be done as: HASH & ((1 << num_bits) - 1)
  VW_DLL_PUBLIC VWStatus vw_workspace_apply_parse_mask(
      const VWWorkspace* workspace_handle, uint64_t hash, uint64_t*, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;

  // Weights
  VW_DLL_PUBLIC VWStatus vw_workspace_get_num_weights(
      const VWWorkspace* workspace_handle, uint32_t* num_weights, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_parameter_width(
      const VWWorkspace* workspace_handle, size_t* parameter_width, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_weights_per_problem(
      const VWWorkspace* workspace_handle, uint32_t* weights_per_problem, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_workspace_get_weight(const VWWorkspace* workspace_handle, size_t index, float** weight,
      size_t* width, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  ...
  VW_DLL_PUBLIC VWStatus vw_workspace_get_weight_iterator_begin(
      const VWWorkspace* workspace_handle, VWWeightIterator** iter, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  // Will fail if you're already at the end.
  VW_DLL_PUBLIC VWStatus vw_weight_iterator_advance(
      VWWeightIterator* iter, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
  VW_DLL_PUBLIC VWStatus vw_weight_iterator_can_advance(
      const VWWeightIterator* iter, bool*, VWErrorInfo* err_info_container) VW_API_NOEXCEPT;
