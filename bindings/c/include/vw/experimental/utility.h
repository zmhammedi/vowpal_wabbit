...
extern "C"
{
  ...
  // Saving
  VW_DLL_PUBLIC VWStatus VWWorkspaceSaveModel(
      const VWWorkspace* workspace_handle, void* context, VWWriteFunc* writer, VWErrorString* err_str_container);

  // Hashing
  // The one passed in options
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetConfiguredAudit(
      const VWWorkspace* workspace_handle, bool* audit, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetConfiguredHashType(
      const VWWorkspace* workspace_handle, VWHashType* hash_type, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetConfiguredHashSeed(
      const VWWorkspace* workspace_handle, uint64_t* hash_seed, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetConfiguredNumBits(
      const VWWorkspace* workspace_handle, uint64_t* num_bits, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetConfiguredHasher(
      const VWWorkspace* workspace_handle, VWHasher** hasher, VWErrorString* err_str_container);

  VW_DLL_PUBLIC VWStatus VWHash(
      const uint8_t* data, size_t length, uint64_t seed, VWHashType type, uint64_t*, VWErrorString* err_str_container);

  // SPEEDY overloads for direct hashing against the type
  VW_DLL_PUBLIC VWStatus VWHashAll(
      const uint8_t* data, size_t length, uint64_t seed, uint64_t*, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWHashString(
      const char* data, size_t length, uint64_t seed, uint64_t*, VWErrorString* err_str_container);

  // Can also be done as: HASH & ((1 << num_bits) - 1)
  VW_DLL_PUBLIC VWStatus VWWorkspaceApplyParseMask(
      const VWWorkspace* workspace_handle, uint64_t hash, uint64_t*, VWErrorString* err_str_container);

  // Weights
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetNumWeights(
      const VWWorkspace* workspace_handle, uint32_t*, VWErrorString* err_str_container);
  ...
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetModelParameter(const VWWorkspace* workspace_handle, size_t index, float** weight,
      size_t* width, VWErrorString* err_str_container);
  ...
  VW_DLL_PUBLIC VWStatus VWWorkspaceGetWeightIteratorBegin(
      const VWWorkspace* workspace_handle, VWWeightIterator** iter, VWErrorString* err_str_container);
  // Will fail if you're already at the end.
  VW_DLL_PUBLIC VWStatus VWWeightIteratorAdvance(VWWeightIterator* iter, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWeightIteratorCanAdvance(
      const VWWeightIterator* iter, bool*, VWErrorString* err_str_container);
  VW_DLL_PUBLIC VWStatus VWWeightIteratorDereference(
      const VWWeightIterator* iter, size_t* index, float** weight, size_t* width, VWErrorString* err_str_container);
