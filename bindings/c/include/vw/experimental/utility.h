...
extern "C"
{
  ...
  VW_DLL_PUBLIC vw_status vw_hash(
  ...
  VW_DLL_PUBLIC vw_status get_model_parameter(
      const vw_workspace* all, size_t index, float** weight, size_t* width, vw_err_str*);
  VW_DLL_PUBLIC vw_status set_weight(
  ...
  VW_DLL_PUBLIC vw_status weight_iterator_deref(
