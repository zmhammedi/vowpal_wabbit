...
void inner_freegrad_update_after_prediction(freegrad_update_data& d, float x, float& wref) {
  ...
  if (d.FG->restart && w[BT]/w[B1]>w[S]+2) {
  ...
  // TODO change this
  if (bt>0)
      w[S] += std::min(fabs_g,bt)/bt ;//std::fabs(clipped_gradient)/(bt*ht);
}
...
base_learner* freegrad_setup(options_i& options, vw& all)
{
  ...
  // TODO change the name of radius
